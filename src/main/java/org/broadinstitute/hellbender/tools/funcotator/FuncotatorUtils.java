...
//        if ( seqComp.getAlignedCodingSequenceReferenceAllele().length() == seqComp.getAlignedCodingSequenceAlternateAllele().length() ) {
     ...
     * @param exonList The {@link List<Locatable>} representing the exons in the transcript in which this variant occurs.
     * @param strandCorrectedRefAllele A {@link String} containing the bases of the reference allele, which are correct for strandedness (i.e. if on a transcript on the {@link Strand#NEGATIVE} strand, the string has already been reverse-complemented).
     * @param strandCorrectedAltAllele A {@link String} containing the bases of the alternate allele, which are correct for strandedness (i.e. if on a transcript on the {@link Strand#NEGATIVE} strand, the string has already been reverse-complemented).
